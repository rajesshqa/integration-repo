...
//        data.stream().forEach(it -> ReportUtil.addTestStepLog(it.toString()));
    ...
    public void MulesoftEventReceived() {
        String csvFilePath = Util.properties.getProperty("event_status_csv_file_path");
        CsvUtil.getCsvHandle(csvFilePath);
        List<EventStatus> data = CsvUtil.getEventStatusData();
        boolean result = Boolean.parseBoolean(data.get(0).getMulesoftEventReceived());
        ReportUtil.addTestStepLog("CP_Forecast__c.Mulesoft_Event_Received__c: " +result);
...
//        ReportUtil.addTestStepLog("CP_Forecast__c.Mulesoft_Processed_On__c: " + LocalDateTime.now());
        ReportUtil.addTestStepLog("CP_Forecast__c.Mulesoft_Processed_On__c: " + data.get(0).getMulesoftProcessedOn());
    }
    ...
    public void MulesoftProcessedSuccess() {
        ...
        CsvUtil.getCsvHandle(csvFilePath);
        List<EventStatus> data = CsvUtil.getEventStatusData();
        boolean result = Boolean.parseBoolean(data.get(0).getMulesoftProcessedSuccess());
        ReportUtil.addTestStepLog("CP_Forecast__c.Mulesoft_Processed_Success__c: " + result);
        Assert.assertTrue(result);
    }
